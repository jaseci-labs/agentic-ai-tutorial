...
- Each ability in one walker needs a **unique name** — two `can respond with ...`
  blocks in the same walker collide (`E0076: Duplicate method`). Name them per
  node: `can route_web with WebNode entry`, `can route_ml with MLNode entry`.
...
| `def fn(...) by llm();` | Use `def:pub` — a plain `def` by-llm returns `None` under `jac run` |
...
| Collecting results after `visit` in the same ability | `visit` is deferred — collect at a later node |
